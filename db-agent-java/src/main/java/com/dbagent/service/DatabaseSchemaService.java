...
/**
 * 数据库表结构提取服务
 * 通过查询 information_schema 获取数据库中所有表的元数据
 */
@Slf4j
@Service
public class DatabaseSchemaService {
    ...
    public DatabaseSchemaService(@Qualifier("primaryJdbcTemplate") JdbcTemplate primaryJdbcTemplate) {
    ...
    private List<String> getAllTableNames(String databaseName) {
        ...
        log.info(
                "表名查询完成 databaseName={}, tableCount={}, costMs={}",
                databaseName,
                tableNames.size(),
                System.currentTimeMillis() - startTime
        );
    ...
    public TableSchema getTableSchema(String databaseName, String tableName) {
        ...
        log.info(
                "字段结构查询完成 databaseName={}, tableName={}, columnCount={}",
                databaseName,
                tableName,
                columnRows.size()
        );
